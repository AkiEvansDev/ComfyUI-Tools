...
class ResBlockEmb(nn.Module):
    def __init__(self, channels, emb_channels, dropout=0, out_channels=None, use_conv=False, use_scale_shift_norm=False, kernel_size=3, exchange_temb_dims=False, skip_t_emb=False):
        super().__init__()
        ...
        self.in_layers = nn.Sequential(normalization(channels), nn.SiLU(), nn.Conv2d(channels, self.out_channels, kernel_size, padding=padding),)
        ...
        if self.skip_t_emb:
            ...
            self.emb_layers = nn.Sequential(nn.SiLU(), nn.Linear(emb_channels, self.emb_out_channels,),)

        self.out_layers = nn.Sequential(normalization(self.out_channels), nn.SiLU(), nn.Dropout(p=dropout), zero_module(nn.Conv2d(self.out_channels, self.out_channels, kernel_size, padding=padding,)),)

        if self.out_channels == channels:
            self.skip_connection = nn.Identity()
        elif use_conv:
            self.skip_connection = nn.Conv2d(channels, self.out_channels, kernel_size, padding=padding)
        else:
...
class LatentResizer(nn.Module):
    ...
    def load_model(cls, filename, device="cpu", dtype=torch.float32, dropout=0):
        ...
        resizer = cls(in_blocks=in_blocks, out_blocks=out_blocks, channels=channels, dropout=dropout, attn=(out_tfs != 0),)
...
class UpscaleLatentBy:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "latent": ("LATENT",),
                "scale_by": ("FLOAT", {"default": 1.50, "min": 1.00, "max": 2.00, "step": 0.05}),
            },
        }
